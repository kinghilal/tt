...
client.on('message', message => {
    ...
    if(!message.channel.guild) return message.channel.send('**This Command Only For Servers !**')
            if (!message.member.hasPermission('MANAGE_ROLES')) return message.channel.send(`**${message.author.username} You Dont Have** ``MANAGE_ROLES`` **Premission**`);
                     ...
                     message.guild.createRole({ name: "مشرف", color: "#ffffff", permissions: [] })
...
client.on("message", async message => {
    if(message.content.startsWith(prefix + "help")) {
        let help = new Discord.RichEmbed()
            ...
            لكي تطرد شخص من السيرفر تكتب : *طرد{prefix}
			لكي تبند شخص كم السيرفر تكتب : *باند}{prefix}
			لكي تمسح المحدثة : *clear {prefix}
         لكي تصنع رولات تكت : *createrolear {prefix}
		   الاومر
		   {prefix} channels*
		  users*{prefix}
		  guilds*{prefix}
